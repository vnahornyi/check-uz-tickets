...
bot.start(async (ctx: Context) => {
    ...
    await ctx.replyWithHTML('Вітаю! Ось головне меню. Оберіть дію:', { reply_markup: mainMenuKeyboard() } as any);
});
...
bot.command('removelink', (ctx: Context) => {
    const parts = (ctx.message && 'text' in ctx.message) ? ctx.message.text.split(' ') : [];
    const link = parts[1];
    if (!link) return ctx.reply('Будь ласка, вкажіть посилання: /removelink <link>');
...
bot.on('text', async (ctx: Context) => {
    ...
    if (state && state.step === 'awaiting_link') {
        if (!text || !urlPattern.test(text)) {
            return ctx.reply('Будь ласка, надішліть коректне посилання або натисніть "Back to main".', { reply_markup: backToMainButton() } as any);
        }
...
bot.on('callback_query', async (ctx: Context) => {
    ...
    try {
        if (data.startsWith('remove:')) {
            ...
            if (removed) {
                await ctx.answerCbQuery('Посилання видалено');
                try {
                    await ctx.editMessageText('Посилання видалено');
                } catch (_e) {}
            } else {
                await ctx.answerCbQuery('Не вдалося видалити посилання', { show_alert: true });
            }
            ...
            redis.redisClient.publish('trackLinks', JSON.stringify({ userId: String(row.telegram_id) }));
            // update message to show check is in progress
            ...
            try {
                await ctx.editMessageText(statusText, { parse_mode: 'HTML', reply_markup: { inline_keyboard: keyboard } } as any);
            } catch (_e) {}
            await ctx.answerCbQuery('Запит на перевірку надіслано');
                } else if (data.startsWith('absent:')) {
            const id = Number(data.split(':')[1]);
            try {
                const { pool } = await import('../db');
                const cooldownMinutes = Number(process.env.ABSENT_COOLDOWN_MINUTES) || 5; // default 5 minutes
                ...
                try {
                    await ctx.editMessageText(text, { parse_mode: 'HTML', reply_markup: { inline_keyboard: keyboard } } as any);
                } catch (_e) {}
                // show main menu after action
                await showMainMenu(ctx);
                await ctx.answerCbQuery('Посилання позначено як відсутнє');
            } catch (err) {
                console.error('Failed to mark absent', err);
                await ctx.answerCbQuery('Не вдалося позначити посилання', { show_alert: true });
            }
        }
    } catch (err) {
        console.error('Callback handling error', err);
