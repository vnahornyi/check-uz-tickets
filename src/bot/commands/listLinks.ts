  ...
  try {
    ...
    for (let idx = 0; idx < limit; idx++) {
      ...
      const lastChecked = row.last_checked_at
        ...
        : "";
      const text = `${statusEmoji} <b>${idx + 1}.</b> ${
        row.link
      } ${notified}${lastChecked}`;
      const keyboard = [
        [{ text: "Видалити ❌", callback_data: `remove:${row.id}` }],
        [
          {
            text: "Позначити як відсутнє 🚫",
            callback_data: `absent:${row.id}`,
          },
        ],
      ];
    ...
    if (links.length > 5) {
      const moreMsg = await ctx.reply(
        `ℹ️ Показано перші 5 з ${links.length} посилань.`
      );
      // include that message id as well
      setUserState(String(ctx.from?.id ?? ""), {
        messages: sentMessageIds.concat([moreMsg.message_id]),
      });
    }
  } catch (error) {
    console.error("Error retrieving links:", error);
    ctx.reply(
      "❌ Виникла помилка при отриманні ваших посилань. Спробуйте ще раз."
    );
  }
};
